...
class SensorSupplyEventCorrelator:
    ...
    def load_datasets(
        self, refill_df: pl.DataFrame, fuel_supply_df: pl.DataFrame
    ) -> None:
        """
        Carga los datasets de refill y fuel supply desde archivos CSV.

        """
        self.refill_df = refill_df.clone()
        self.fuel_supply_df = fuel_supply_df.clone()
    ...
    def _prepare(self) -> None:
        ...
        self.refill_df = (
            self.refill_df.select(["DeltaFuel", "TimeStamp", "BeforeAvg", "AfterAvg"])
            .sort(["TimeStamp"])
    ...
    def _calculate_optimal_adjustment(self, df: pl.DataFrame) -> Tuple[float, float]:
        """
        Calcula dos valores óptimos para ajustar DeltaFuel y minimizar fuel_discrepancy:
        - subtract_value: valor a restar cuando DeltaFuel > FuelLevelLiters
        - add_value: valor a sumar cuando DeltaFuel < FuelLevelLiters
        ...
        higher_data: pl.DataFrame = valid_data.filter(
            pl.col("DeltaFuel") > pl.col("FuelLevelLiters")
        )
        subtract_value = 0.0

        if higher_data.height > 0:

            DeltaFuel_higher: ndarray[tuple[int], Any] = (
                higher_data.select("DeltaFuel").to_numpy().flatten()
            )
            ...
            def objective_function_subtract(adjustment_value) -> float:
                ...
                adjusted_DeltaFuel = DeltaFuel_higher - abs(
                    adjustment_value
                )  # Siempre restar
                discrepancy: Any = np.abs(adjusted_DeltaFuel - fuel_level_higher)
                return float(np.mean(discrepancy))

            # Rango de búsqueda para restar
            max_difference = np.max(DeltaFuel_higher - fuel_level_higher)
        ...
        if lower_data.height > 0:
            print(
                f"Analizando {lower_data.height} registros donde DeltaFuel < FuelLevelLiters"
            )

            DeltaFuel_lower: ndarray[tuple[int], Any] = (
                lower_data.select("DeltaFuel").to_numpy().flatten()
            )
            ...
            def objective_function_add(adjustment_value) -> float:
                ...
                adjusted_DeltaFuel = DeltaFuel_lower + abs(
                    adjustment_value
                )  # Siempre sumar
                discrepancy = np.abs(adjusted_DeltaFuel - fuel_level_lower)
                return float(np.mean(discrepancy))

            # Rango de búsqueda para sumar
            max_difference = np.max(fuel_level_lower - DeltaFuel_lower)
            search_bounds_add = (
                0,
                max_difference * 1.5,
            ...
            print("No hay registros donde DeltaFuel < FuelLevelLiters")

        equal_data = valid_data.filter(pl.col("DeltaFuel") == pl.col("FuelLevelLiters"))
        if equal_data.height > 0:
            print(
                f"Encontrados {equal_data.height} registros donde DeltaFuel == FuelLevelLiters (perfectos)"
            )

        print(f"\nRESUMEN DE OPTIMIZACIÓN:")
        print(
            f"  Valor a restar cuando DeltaFuel > FuelLevelLiters: {subtract_value:.6f}"
        )
        print(f"  Valor a sumar cuando DeltaFuel < FuelLevelLiters: {add_value:.6f}")
    ...
    def correlate_events(self) -> pl.DataFrame:
        ...
        # forzar la precision
        supplies = supplies.with_columns(
            pl.col("SupplyTimeStamp").dt.cast_time_unit("ms")
        )

        refills = refills.with_columns(
            pl.col("RefillTimeStamp").dt.cast_time_unit("ms")
        )

        best_matches = best_matches.with_columns(
            pl.col("RefillTimeStamp").dt.cast_time_unit("ms"),
            pl.col("SupplyTimeStamp").dt.cast_time_unit("ms"),
            pl.col("DeltaFuel").cast(pl.Float32),
            pl.col("BeforeAvg").cast(pl.Float32),
            pl.col("AfterAvg").cast(pl.Float32),
            pl.col("FuelLevelLiters").cast(pl.Float32),
            pl.col("time_score").cast(pl.Float32),
            pl.col("fuel_diff_score").cast(pl.Float32),
            pl.col("origin_score").cast(pl.Float32),
            pl.col("shift_score").cast(pl.Float32),
            pl.col("total_score").cast(pl.Float32),
        )

        # Obtener supplies o refill no emparejados
        ...
        # castear a la fuerza
        # Convertir matched_supplies_series de us → ms
        matched_supplies_series = matched_supplies_series.dt.cast_time_unit("ms")

        # Convertir matched_refills_series de us → ms
        matched_refills_series = matched_refills_series.dt.cast_time_unit("ms")
        ...
        unmatched_supplies = unmatched_supplies.with_columns(
            ...
            pl.lit(None).alias("BeforeAvg"),
            pl.lit(None).alias("AfterAvg"),
        ...
        initial_df = (
            initial_df.with_columns(
                (pl.col("FuelLevelLiters") - pl.col("AfterAvg"))
                .abs()
                .alias("distance_after"),
                (pl.col("FuelLevelLiters") - pl.col("BeforeAvg"))
                .abs()
                .alias("distance_before"),
                (pl.col("FuelLevelLiters") - pl.col("DeltaFuel"))
                .abs()
            ...
            .with_columns(
                ...
                .then(pl.col("BeforeAvg"))
                .otherwise(pl.col("DeltaFuel"))
                .alias("DeltaFuel")
            )
            .with_columns(
                pl.when(
                ...
                .then((pl.col("DeltaFuel") - pl.col("FuelLevelLiters")).abs())
        ...
        self.merged_df = initial_df.with_columns(
            # Aplicar ajuste solo donde DeltaFuel es menor a registros
            pl.when(
                (pl.col("DeltaFuel").is_not_null())
                & (pl.col("FuelLevelLiters").is_not_null())
                & (pl.col("DeltaFuel") < pl.col("FuelLevelLiters"))
            ...
            .then(pl.col("DeltaFuel") + add_value)
            .when(
                (pl.col("DeltaFuel").is_not_null())
                & (pl.col("FuelLevelLiters").is_not_null())
                & (pl.col("DeltaFuel") > pl.col("FuelLevelLiters"))
            ...
            .then(pl.col("DeltaFuel") - subtract_value)
            .otherwise(pl.col("DeltaFuel"))
            .alias("DeltaFuel")
        ).with_columns(
    ...
    def correlate_anomalies(
        ...
        if best_matches_list:
            ...
            # Ajustar DeltaFuel según truck_id y ajustes
            truck_id = (
            ...
            completed = (
                ...
                .with_columns(
                    pl.when(
                        # Caso 1: DeltaFuel < FuelLevelLiters y diferencia >= threshold
                        (pl.col("DeltaFuel").is_not_null())
                        & (pl.col("FuelLevelLiters").is_not_null())
                        & (pl.col("DeltaFuel") < pl.col("FuelLevelLiters"))
                        & (
                    ...
                    .then(pl.col("DeltaFuel") + add_value)
                    .when(
                        # Caso 2: DeltaFuel > FuelLevelLiters y diferencia >= threshold
                        (pl.col("DeltaFuel").is_not_null())
                        & (pl.col("FuelLevelLiters").is_not_null())
                        & (pl.col("DeltaFuel") > pl.col("FuelLevelLiters"))
                        & (
                    ...
                    .then(pl.col("DeltaFuel") - subtract_value)
                    .otherwise(pl.col("DeltaFuel"))  # Mantener valor si está cerca
                    .alias("DeltaFuel")
                )
                .with_columns(
            ...
            completed = pl.DataFrame(
                schema={
                    ...
                    "DeltaFuel": pl.Float64,
                    "BeforeAvg": pl.Float64,
                    "AfterAvg": pl.Float64,
                    "Origin": pl.Utf8,
