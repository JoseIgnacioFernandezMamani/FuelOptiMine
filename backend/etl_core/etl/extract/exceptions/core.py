...
class ExtractionError(Exception):
    ...
    def __str__(self) -> str:
        ...
        tb = self._get_relevant_traceback()
        ...
        if tb:
            ...
            context = self._get_code_context(filename, lineno)
            if context:
                original_msg += f"\n\n   ==== CODE CONTEXT ====\n{context}"
    ...
    def _get_relevant_traceback(self) -> Optional[tuple]:
        ...
        tb = self.cause.__traceback__ if self.cause else self.__traceback__
    ...
    def _get_code_context(self, filename: str, lineno: int, context_lines: int = 3) -> str:
