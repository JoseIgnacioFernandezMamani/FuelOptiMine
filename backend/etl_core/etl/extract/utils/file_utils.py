...
from etl_core.etl.extract.models.schemas import SUPPORTED_FORMATS, DATASET_TYPES
...
def validate_truck_exists(
    base_dir: Path, dataset: str, truck: str, file_extension: str
) -> bool:
    ...
    if not find_matching_files(patterns):
        raise FileNotFoundError(
            f"No files found for truck {truck} in {dataset} dataset"
        )
    return True


def generate_file_patterns(
    base_dir: Path, dataset: str, data_type: str, truck: str, file_extension: str
) -> List[str]:
    ...
    return [
        str(
            ...
            / "*"
            / f"{truck}_*.{file_extension.lstrip('.')}"
        )
    ]
