...
import polars as pl
from transform.implementation.fuel_supply.transformer import FuelSupplyTransformer
...
def run_fuel_supply_etl_pipeline():
    ...
    try:
        ...
        if not raw_data or data_type not in raw_data:
            raise ValueError(f"Datos de tipo '{data_type}' no encontrados o estructura inválida")
        ...
        df_clean = transformer.run_transform(df_raw)  # Solo ejecuta hasta normalize_and_validate
        
        if df_clean is None or df_clean.is_empty():
            raise RuntimeError("La limpieza/normalización devolvió datos vacíos")

        # 3. Validación de campos obligatorios (ahora desde la propiedad) ⚡
        mandatory_columns = transformer.mandatory_columns  # ⚡ Usa la propiedad
        missing_columns = [col for col in mandatory_columns if col not in df_clean.columns]
        if missing_columns:
