...
import polars as pl
from transform.implementation.time_model.transformer import TimeModelTransformer
    ...
    try:
        ...
        for col, dtype in schema_checks.items():
            if df_clean.schema[col] != dtype:
                raise TypeError(f"Tipo incorrecto en {col}: Esperado {dtype}, Obtenido {df_clean.schema[col]}")

        # 6. Muestra de datos normalizados
        print("\n📄 Muestra de datos normalizados:")
        print(df_clean.select([
            'ShiftDate', 'TimeStamp', 'Equipment', 
            'Status', 'RecordDuration'
        ]).head(3))
