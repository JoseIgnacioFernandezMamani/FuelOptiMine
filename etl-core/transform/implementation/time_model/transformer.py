...
class TimeModelTransformer(BaseTransformer):
    ...
    def _calculate_time_since_last_event(self) -> pl.Expr:
        ...
        return pl.col("TimeStamp").diff().over(["Equipment", "EventType"]).dt.total_seconds().alias("time_since_last_event")
    ...
    def _create_rolling_features(self, metric: str) -> List[pl.Expr]:
        ...
        return [
            pl.col(metric).rolling_mean(window_size, min_periods=1).over("Equipment").alias(f"{metric}_rolling_{window_size}s")
    ...
    def _calculate_equipment_utilization(self) -> pl.Expr:
        ...
        operating_time = pl.col("Status").is_in(["Producción", "Transporte"]).cast(pl.Int64)
        return (
            operating_time.rolling_mean(3600).over("Equipment") * 100
    ...
    def _predict_remaining_operating_time(self) -> pl.Expr:
        ...
        return (
            ...
            pl.col("FuelLevelLiters").diff().abs().rolling_mean(300).over("Equipment")
        ).clip(lower_bound=0).alias("predicted_remaining_time")
        )
