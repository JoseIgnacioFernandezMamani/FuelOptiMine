...
class SensorTransformer(BaseTransformer):
    ...
    def _calculate_fuel_consumption_rate(self, time_unit: str="hour") -> pl.Expr:
        ...
        if time_unit not in time_factors:
            raise ValueError(f"Unidad de tiempo no válida: {time_unit}. Debe ser una de {list(time_factors.keys())}.")
        
        factor = time_factors[time_unit.lower()]

        time_diff = (
            (pl.col("TimeStamp").diff(1).over("Equipment").dt.total_seconds() / factor) # calculo de diferencia de tiempo en horas
            .fill_null(0)
    ...
    def _calculate_slope_percent(self) -> pl.Expr:
        ...
        return (
            ...
            ((pl.col("Latitude").diff(1).over("Equipment")**2 + pl.col("Longitude").diff(1).over("Equipment")**2).sqrt() + 1e-9) * 100
            ).fill_nan(0).alias("slope_percent")
        )
    ...
    def _calculate_efficiency_ratio(self) -> pl.Expr:
        ...
        return (
            ...
            pl.col("fuel_consumption").clip(lower_bound=0.1)  # Evitar división por cero
        ).fill_null(0).alias("efficiency_ratio")
