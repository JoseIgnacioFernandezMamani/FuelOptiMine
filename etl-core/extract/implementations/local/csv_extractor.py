...
from extract.interfaces.local import IFileExtractor
...
from extract.models.schemas import COLUMN_MAPPING, DATASET_TYPES, SUPPORTED_FORMATS, FUEL_SUPPLY
        ...
        try:
            ...
            for file in dispatch_files:
                try:
                    ...
                    df = pl.from_pandas(pandas_df).filter(
                        (pl.col("Descripcion").is_in(self.VALID_TRUCK_MODELS)) &
                        (pl.col("Veh").str.extract(r"^(\d{3})", 0) == self.truck_number)
                    ).with_columns(
        ...
        try:
            ...
            for data_type in COLUMN_MAPPING:
                try:
                    ...
                    # Schema validation
                    missing_cols = [
                        col for col in COLUMN_MAPPING[data_type] 
